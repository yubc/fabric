...
function networkUp () {
  ...
  echo "###############################################################"
  docker exec Org4cli ./scripts/step2org3.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT
  if [ $? -ne 0 ]; then
  ...
  docker exec Org4cli ./scripts/testorg3.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT
  if [ $? -ne 0 ]; then
...
function networkDown () {
  docker-compose -f $COMPOSE_FILE_ORG3 down --volumes
